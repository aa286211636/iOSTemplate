...
typedef void(^SegmentIndexBlock)(NSUInteger ,UIButton *);
int const CustomSegmentHeight = 46;//可根据项目需求设置高度
...
@property (nonatomic, copy)   SegmentIndexBlock indexBlock;
...
- (void)segmentBasicSetting {
    ...
    _segmentHeight = CustomSegmentHeight;
    _minItemSpace = 40.;
...
- (void)scrollIndicateView {
    ...
    [UIView animateWithDuration:_duration delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        if (self->_style == CustomSegmentStyleDefault) {
            self->_indicateView.frame = CGRectMake(CGRectGetMinX(self->_selectedButton.frame) + self->_buttonSpace, CGRectGetMinY(self->_indicateView.frame), titleSize.width, self->_indicateHeight);
        } else {
            self->_indicateView.frame = CGRectMake(CGRectGetMinX(self->_selectedButton.frame), CGRectGetMinY(self->_indicateView.frame), [self widthAtIndex:index], self->_indicateHeight);
        }
    } completion:nil];
}
...
- (void)adjustOffsetXToFixIndicatePosition:(CGFloat)offsetX {
    ...
    CGFloat originMovedX = _style == CustomSegmentStyleDefault? (CGRectGetMinX(_selectedButton.frame) + _buttonSpace) : CGRectGetMinX(_selectedButton.frame);
    CGFloat targetMovedWidth = [self widthAtIndex:currentIndex];//需要移动的距离
    
    CGFloat targetButtonWidth = _style == CustomSegmentStyleDefault? ([self widthAtIndex:buttonIndex] - 2 * _buttonSpace) : [self widthAtIndex:buttonIndex]; // 这个会影响width
    CGFloat originButtonWidth = _style == CustomSegmentStyleDefault? ([self widthAtIndex:[self index]] - 2 * _buttonSpace) : [self widthAtIndex:[self index]];
...
- (void)setStyle:(CustomSegmentStyle)style {
    _style = style;
    if (style == CustomSegmentStyleFlush) {
